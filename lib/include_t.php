...
class tMethod extends method{
	...
	public function getTestType($where){
		...
		$sql .= " ,max(t.type) as type";
		$sql .= " ,u.name,u.privacy_flg ";
		$sql .= " ,u.login_id ";
		$sql .= " ,u.privacypolicy_text ";
		$sql .= " ,u.privacypolicy ";
		$sql .= " ,t.fin_disp";
		$sql .= " ,t.enq_status";
		$sql .= " ,t.id ";
		...
		$sql .= " FROM ";
		$sql .= " t_test as t ";
		$sql .= " LEFT JOIN (SELECT id,name,login_id,privacy_flg,privacypolicy,privacypolicy_text FROM t_user) as u ON u.id=t.customer_id";
	...
	public function getTestLink($where){
		...
		$sql .= " FROM ";
		$sql .= " t_test as t";
		$sql .= " LEFT JOIN (SELECT id,name,exam_id,exam_state,testgrp_id,type,complete_flg,test_id,tensaku_status FROM t_testpaper ) as tt ON t.test_id=tt.testgrp_id ";
		$sql .= " WHERE ";
	...
	public function getJugData($mem,$types = 52){
      ...
      }elseif($types == 87 || $types == 90){
        $jugtable = "jug_boss_text6";
	    }else{
	        $jugtable = "jug_boss_text2";
	    }
	    $sql = "
				SELECT a.* FROM (
				SELECT
				...
				FROM
					jug_boss as j1
					LEFT JOIN jug_member as jm1 ON jm1.id = j1.bossid
					LEFT JOIN ".$jugtable." as jb ON jb.bossid = jm1.id AND type = 2 AND jb.jmid = '".$mem[ 'id' ]."'
				WHERE
					j1.jmid = ".$mem[ 'id' ]."
					AND j1.bossid <> 0
				UNION ALL
				...
				FROM
					jug_boss as j2
					LEFT JOIN jug_member as jm2 ON jm2.id = j2.jmid
					LEFT JOIN ".$jugtable." as jb ON jb.bossid = 0 AND type = 1 AND jb.jmid = '".$mem[ 'id' ]."'
				WHERE
					j2.bossid = ".$mem[ 'id' ]."
	...
	public function getJugData2($mem,$types = 52){

...
//			}
	...
	public function interviewCheckRS(){
		...
		$points = array(
			...
			"rs_sentaku"=>array(
				"p"=>$this->rs_sentaku
				,"lv"=>$this->rs_sentakulv
				,"sort"=>3
			),
			"rs_kirikae"=>array(
				"p"=>$this->rs_kirikae
				,"lv"=>$this->rs_kirikaelv
				,"sort"=>4
			),
			// "rs_jyoho"=>array(
			// 	"p"=>$this->rs_jyoho
	...
	public function getStX(){
		...
		$ten = $this->tenX;
		if($ten == 2){$keypt = $keyp[0];}else
		if($ten == 3){$keypt = $keyp[1];}else
		if($ten == 4){$keypt = $keyp[2];}else
		if($ten == 5){$keypt = $keyp[3];}else
		if($ten == 6){$keypt = $keyp[4];}else
		if($ten == 7){$keypt = $keyp[5];}else
		if($ten == 8){$keypt = $keyp[6];}
		return $keypt;
	}
	...
	public function getUserDataExcel($where,$flg=""){
		...
		if($flg){
			$sql .= " type IN(1,2,12,59,72,82) AND ";
		}
		$sql .= " 1=1 ";

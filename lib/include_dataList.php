...
class dataListMethod extends method{
        public function getPdfLog($where){
            $testid = $where[ 'test_id' ];
            $sql = "SELECT "
                    . " COUNT(id) as cnt,"
                    . " exam_id "
                    . " FROM "
	...
	public function getPdfLogUse($where){
     $sql = "";
     $sql = "SELECT pdf_log_use FROM t_test WHERE test_id = ".$where[ 'testgrp_id' ];
     $sql .= " GROUP BY test_id";
     $stmt2 = $this->db->prepare($sql);
     $stmt2->execute();
     $result2 = $stmt2->fetch(PDO::FETCH_ASSOC);
     return $result2[ 'pdf_log_use' ];
 }
	public function getTestlist($where,$lines){
     ...
     $this->pdfloguse = $result2[ 'pdf_log_use' ];
		...
		$sql .= " tt.mail,";
		$sql .= " tt.ampflag,";
    if($result2[ 'pdf_log_use' ] == 1){
        $sql .= " COUNT(l.id) as logcount,";
    }
		$sql .= " CASE ";
  ...
  if($result2[ 'pdf_log_use' ] == 1){
      $sql .= " ,(SELECT pdfdownload FROM t_test WHERE id=".$testgrp_id." AND type=0 ) as pdf";
		...
		if($pid){
			$sql .= " LEFT JOIN (SELECT test_id,customer_id,partner_id ,stress_flg FROM t_test ) as t ON tt.testgrp_id = t.test_id AND t.customer_id=".$cid." AND t.partner_id=".$pid;
		}else{
			$sql .= " LEFT JOIN (SELECT test_id,customer_id,partner_id ,stress_flg FROM t_test ) as t ON tt.testgrp_id = t.test_id AND t.customer_id=".$cid;
		}
  
   if($result2[ 'pdf_log_use' ] == 1){
      $sql .= " LEFT JOIN log_pdf as l ON l.exam_id=tt.exam_id AND l.test_id=".$testgrp_id." ";
   }
		...
		$sql .= " 1=1 ";
		$sql .= " GROUP BY tt.id ORDER BY tt.number ASC,tt.type ASC  ";
	...
	public function getTensakuStsEd($where){
        ...
        public function getTestData2($where){
            $test_id = $where[ 'test_id' ];
            $sql = "SELECT "
		...
		public function getTestDatapdfslice($id){
			$sql = "
				SELECT 
					pdf_slice
				FROM

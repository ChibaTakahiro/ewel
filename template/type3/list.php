...
?>
		<?php if($cdata[0]['password_editdate'] < $lastyear ):?>
			<br clear=all />
...
<?php if($basetype != 2 && $customer_file_upload == 0 || $basetype == 1){ ?>
				<li><a href="/index/download/"><?=$str7?></a></li>
<?PHP } ?>

<?php if($basetype == 3 && $customer_file_upload == 1){ ?>
				<li><a href="/index/tmp/">ファイル共有</a></li>
<?PHP } ?>
		...
		<div id="searchBox">
			<form action="/index/search/" name="sform" method="POST">
				...
				<div class="searchItems">
					<div class="itemCell">
						ID：<input type="text" name="exam_id" class="exam_id" value="<?=$_REQUEST[ 'exam_id' ]?>">
					</div>
					<div class="itemCell">
						<?=$str9?>：<input type="text" name="kana" value="<?=$_REQUEST[ 'kana' ]?>" >
					</div>
					<div class="itemCell">
						<?=$str24?>：
				...
				<div class="searchItems">
					...
					<div class="itemCell">
						<?=$str22?>：<input type="text" name="memo1" value="<?=$_REQUEST['memo1']?>">
					</div>
					<div class="itemCell">
						<?=$str23?>：<input type="text" name="memo2" value="<?=$_REQUEST['memo2']?>">
					</div>
				</div>

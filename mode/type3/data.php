...
if(is_numeric($sec)){
    ...
    $pwhere = [];
    $pwhere[ 'id' ] = $sec;
    $pdfslice = $obj->getTestDatapdfslice( $pwhere );
    
$where2 = array();
$where2[ 'testgrp_id' ] = $sec;
$pdfloguse = $obj->getPdfLogUse($where2);
}

//pdflogdata
//$pdflog = $obj->getPdfLog($where);
...
if($_REQUEST[ 'ampflag' ]){
        ...
        $where = array();
        $where[ 'where' ][ 'id'         ] = $id;
        $where[ 'edit'  ][ 'ampflag' ] = 1;
        $where[ 'edit'  ][ 'ampdate' ] = date('Y-m-d');
        $tbl = "t_testpaper";
        $db->editUserData($tbl,$where);
...
//CRES検査の時は別ページにリダイレクト

if($tykey['75']){
        header("Location:/index/cres/".$sec);
        exit();
}

if($tykey[52] || $tykey[60] || $tykey[86] || $tykey[87] || $tykey[62]  || $tykey[67] || $tykey[68]){
//評価検査のとき
...
if($_REQUEST[ 'lists' ]){
    ...
    */
	...
	if($_REQUEST[ 'lists' ]){
		...
		$offset = sprintf("%d",(int)$limit*(int)$_REQUEST[ 'pages' ]);
		$where = array();
		$where[ 'pid'        ] = $ptid;
		$where[ 'cid'        ] = $id;
		$where[ 'testgrp_id' ] = $sec;
		...
		$where[ 'offset'     ] = $offset;
                ...
                $kanasearch = $_REQUEST[ 'kanasearch' ];
    ...
    $where[ 'pdfbutton' ] = $pdfbutton;
		$list = $obj->getTestlist($where,$wheres);
  $pdfloguse = $obj->pdfloguse;
		...
		foreach($list as $key=>$val){
			...
			$data[ $val[ 'number' ] ]['mail'                ][$val[ 'type' ]] = $val[ 'mail'            ];
                        $data[ $val[ 'number' ] ]['logcount'            ] = $val[ 'logcount' ];
                        $data[ $val[ 'number' ] ]['ampflag'            ] = $val[ 'ampflag' ];
		...
		//	$html .= "<td class='center' ><input type='checkbox' name='del'  value='".$val[exam_id]."' id='del_".$val[exam_id]."' class='delchecks' ".$val[ 'disabled' ]." ></td>";
			...
			foreach($val[ 'type' ] as $k=>$v){
				...
				//------------------------------------------------------------------------------------------------
				//IQ検定で$iqes[ID]がある時
                                if($iqes[ $val[ 'exam_id' ]] && $val['type'][$v] == 11 && $val[ 'exam_state' ][$v] == 0){
					$html .= "<td class='center'>".$dstr1.$math."</td>";
				}else
			...
			$html .= "<td class='left'>".$val[ 'memo2' ]."</td>";
                        ...
                        if($val['type'][$v] == 83 ){
                                $txt = "<input type='checkbox' name='ampccheck' id='ampccheck_".$val[ 'id' ][ $v ]."' value='on' />出力";
                                if($val[ 'ampflag' ] == 1){
                                        $txt = "出力済";
                                }
                                        $html .= "<td class='left'>
                                        <label>
                                        <input type='hidden' name='examid' value='".$val[ 'exam_id' ]."' />
                                        ".$txt."</label>
                                        </td>";
    ...
    if($pdfloguse == 1){
        ...
        $html .= "<td class='left'>".$pdfstr."</td>";
			...
			//}else
			if(!$pdfdisabled  || ($pdfloguse == 0 && $val['complete_flg'][$v] )){
                            $pdfdownloads = "pdfdownloads";
                            if($val[ 'logcount' ]){
                                $pdfdownloads = "";
                            }
				$html .= "<li class='w30' ><a href='/index/pdf/".$sec."/".$val[ 'exam_id' ]."/".$a3."' class='".$makePdf." ".$pdfdownloads."'  id='".$val['exam_id']."-".$val['name']."'>PDF</a>";
                                $html .= "</li>";
				...
				$html .= "<li class='w30' ><p class='disabled'>PDF</p></li>";
                        ...
                        if($types2['licensecard']){
                                $html .= "<li class='w30' ><a href='".D_URL."index/pdf/".$sec."/".$val['exam_id']."/a4/?code=lisence'>証書</a></li>";
                        }
			$html .= "</ul>";
			$html .= "</td>";
		...
		if($ceil > 1){
				...
				if($one < 0) $one = 0;
				$end = $one+5;
				$page .= "<li><a href='/index/data/".$sec."/0' class='pageclick' id='ep-0'>[<<]</a></li>";
				for($i=$one;$i<$end;$i++){
					$cnt = $i+1;
					if($i >= $ceil){

					}else{
						$page .= "<li>[<a href='/index/data/".$sec."/".$i."' class='pageclick' >".$cnt."</a>]</li>";
					}
				}
				$ends = $ceil-1;
				$page .= "<li><a href='/index/data/".$sec."/".$ends."' id='ep-".$ends."' class='pageclick'>[>>]</a></li>";
